...
export default function ExpensesPage() {
  ...
  const load = () => base44.entities.Expense.list("-date").then((data) => { setExpenses(data); setLoading(false); });
  ...
  const handleExport = () => {
    exportToCsv("расходы.csv", ["Тип операции", "Наименование операции", "Категория", "Сумма", "Дата"],
      filtered.map((i) => [i.operation_type, i.operation_name, i.category, i.amount, i.date]));
  };
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-6 gap-4 flex-wrap">
        <h1 className="text-2xl font-heading font-bold text-gray-900">Расходы</h1>
      ...
      <div className="relative mb-4">
        ...
        <Input
          placeholder="Поиск по типу, операции, категории..."
          value={search}
      ...
      {filtered.length === 0 ? (
        <div className="bg-white rounded-xl border border-gray-200 p-12 text-center text-gray-400">
          {search ? "Ничего не найдено" : "Нет записей о расходах"}
        </div>
      ) : (
        <>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-gray-100 text-left text-gray-500">
                <th className="px-4 py-3 font-medium">Тип операции</th>
                <th className="px-4 py-3 font-medium">Наименование операции</th>
                ...
                <React.Fragment key={group.key}>
                  ...
                  {group.items.map((item) => (
                    <tr key={item.id} className="border-b border-gray-50 hover:bg-gray-50/50 transition-colors">
                      <td className="px-4 py-3 text-gray-700">{item.operation_type}</td>
                      <td className="px-4 py-3 font-medium text-gray-900">{item.operation_name}</td>
                      <td className="px-4 py-3 text-gray-700">{item.category || "—"}</td>
                      <td className="px-4 py-3 text-red-500 font-semibold">{fmt(item.amount)} ₽</td>
                ...
                {group.items.map((item) => (
                  <div key={item.id} className="bg-white rounded-xl border border-gray-200 p-4 space-y-2">
                    <div className="flex items-start justify-between gap-2">
                      ...
                      <span className="text-red-500 font-semibold whitespace-nowrap">{fmt(item.amount)} ₽</span>
                    </div>
                    {item.operation_type && <div className="text-sm text-gray-600">{item.operation_type}</div>}
      ...
      <ExpenseForm open={formOpen} onClose={() => setFormOpen(false)} onSave={handleSave} editItem={editItem} />
    </div>
  );
}